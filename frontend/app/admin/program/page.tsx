...
export default function AdminProgramPage() {
  ...
  const { data: exercisesPickerRes, isLoading: exercisesPickerLoading } =
    useQuery<ExercisesResponse>({
      queryKey: ['admin-exercises-picker', exercisePickerFilter],
  ...
  const { mutate: submitProgramDay, isPending: isCreatingDay } = useMutation({
    ...
    onError: () => {
      message.error('Không thể tạo ngày tập. Vui lòng thử lại.');
    },
  });
  ...
  const { mutate: submitProgramDayExercise, isPending: isAddingExercise } =
    useMutation({
      ...
      onError: () => {
        message.error('Không thể thêm bài tập. Vui lòng thử lại.');
      },
    });
  ...
  return (
    <>
      <div className="mb-8 flex flex-wrap items-center gap-3 rounded bg-gray-50 p-4">
        <Search
          allowClear
